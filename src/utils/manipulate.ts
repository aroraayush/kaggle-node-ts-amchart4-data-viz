...
class Manipulator {
  ...
  mergeJSONArr(array: any, groupKey: string) {
    return _(array)
      .groupBy(groupKey)
      .map((objs, key) => ({
        [groupKey]: new Date(parseInt(key.split("-")[0], 10), parseInt(key.split("-")[1], 10) - 1, parseInt(key.split("-")[2], 10)),
  ...
  mergeJSONArrayLarge(array: any, groupKey1: string, groupKey2: string) {
    const group1Merge : any = _.groupBy(array, groupKey1);
    _.forEach(group1Merge, (value, key) => {
      group1Merge[key] = _(group1Merge[key]).groupBy(groupKey2)
                          .map((objs2 : any, key2 : any) => ({
        [groupKey2]: new Date(parseInt(key2.split("-")[0], 10), parseInt(key2.split("-")[1], 10) - 1, parseInt(key2.split("-")[2], 10)),
